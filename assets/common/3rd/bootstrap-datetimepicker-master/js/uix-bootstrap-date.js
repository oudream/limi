define(['jquery', 'util', 'bootstrap', 'bootstrap-dateCN'], function ($, util) {
  ...
  var UixBootstrapDate = function (element, option) {
    var $ele = $(element)
    this.element = $ele
  ...
  $.fn.extend({
    'uixDate': function (params) {
			// 获取参数
      var value, type, readonly, format, disabled
      if (params != undefined) {
      ...
      var template = '<div class="input-append date ' + type + '" data-date-format="' + format + '"  data-date ="' + value + '" data-link-field="' + $(this).attr('id') + '"><input size="16" type="text" name="time" value="' + value + '" ' + readonly + '>'
			//			if(!hideRemove) {
