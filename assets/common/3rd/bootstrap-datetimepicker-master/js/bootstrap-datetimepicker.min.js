(function(a) {
    ...
    if (!('indexOf' in Array.prototype)) {
        ...
        let m=this; this.element=d(k); this.container=j.container||'body'; this.language=j.language||this.element.data('date-language')||'en'; this.language=this.language in e?this.language:this.language.split('-')[0]; this.language=this.language in e?this.language:'en'; this.isRTL=e[this.language].rtl||false; this.formatType=j.formatType||this.element.data('format-type')||'standard'; this.format=c.parseFormat(j.format||this.element.data('date-format')||e[this.language].format||c.getDefaultFormat(this.formatType, 'input'), this.formatType); this.isInline=false; this.isVisible=false; this.isInput=this.element.is('input'); this.fontAwesome=j.fontAwesome||this.element.data('font-awesome')||false; this.bootcssVer=j.bootcssVer||(this.isInput?(this.element.is('.form-control')?3:2):(this.bootcssVer=this.element.is('.input-group')?3:2)); this.component=this.element.is('.date')?(this.bootcssVer===3?this.element.find('.input-group-addon .glyphicon-th, .input-group-addon .glyphicon-time, .input-group-addon .glyphicon-remove, .input-group-addon .glyphicon-calendar, .input-group-addon .fa-calendar, .input-group-addon .fa-clock-o').parent():this.element.find('.add-on .icon-th, .add-on .icon-time, .add-on .icon-calendar, .add-on .fa-calendar, .add-on .fa-clock-o').parent()):false; this.componentReset=this.element.is('.date')?(this.bootcssVer===3?this.element.find('.input-group-addon .glyphicon-remove, .input-group-addon .fa-times').parent():this.element.find('.add-on .icon-remove, .add-on .fa-times').parent()):false; this.hasInput=this.component&&this.element.find('input').length; if (this.component&&this.component.length===0) {
...
} let l=this.bootcssVer===3?c.templateV3:c.template; while (l.indexOf('{iconType}')!==-1) {
    ...
    this.picker.addClass('datetimepicker-rtl'); let i=this.bootcssVer===3?'.prev span, .next span':'.prev i, .next i'; this.picker.find(i).toggleClass(this.icons.leftArrow+' '+this.icons.rightArrow);
    ...
    let p=(j.onRenderDay||function() {
        return [];
    })(n); if (typeof p==='string') {
        p=[p];
    } let o=['day']; return o.concat((p?p:[]));
}; this.onRenderHour=function(n) {
    let p=(j.onRenderHour||function() {
        return [];
    })(n); let o=['hour']; if (typeof p==='string') {
        p=[p];
    } return o.concat((p?p:[]));
}; this.onRenderMinute=function(n) {
    let p=(j.onRenderMinute||function() {
        return [];
    })(n); let o=['minute']; if (typeof p==='string') {
        p=[p];
    ...
    let q=(j.onRenderYear||function() {
        return [];
    })(o); let p=['year']; if (typeof q==='string') {
        q=[q];
    } if (this.date.getUTCFullYear()===o.getUTCFullYear()) {
        p.push('active');
    } let n=o.getUTCFullYear(); let r=this.endDate.getUTCFullYear(); if (o<this.startDate||n>r) {
        p.push('disabled');
    } return p.concat((q?q:[]));
}; this.onRenderMonth=function(n) {
    let p=(j.onRenderMonth||function() {
        return [];
    })(n); let o=['month']; if (typeof p==='string') {
        p=[p];
    } return o.concat((p?p:[]));
    ...
    let i=this.getUTCDate(); if (i===null) {
        return null;
    ...
    this.format=c.parseFormat(j, this.formatType); let i; if (this.isInput) {
        i=this.element;
    } else {
        if (this.component) {
    ...
    let i=this.getFormattedDate(); if (!this.isInput) {
        if (this.component) {
    ...
    } let i=this; this.datesDisabled=d.map(this.datesDisabled, function(k) {
    ...
    if (this.isInline) {
        ...
        let j=0; d('div').each(function() {
            let o=parseInt(d(this).css('zIndex'), 10); if (o>j) {
                j=o;
            }
        }); this.zIndex=j+10;
    } let n, m, l, k; if (this.container instanceof d) {
    ...
    } let i=document.body.clientWidth||window.innerWidth; if (l+220>i) {
        l=i-220;
    ...
    let i, j=false; if (arguments&&arguments.length&&(typeof arguments[0]==='string'||arguments[0] instanceof Date)) {
    ...
    } this.picker.find('.datetimepicker-days tbody').empty().append(r.join('')); r=[]; let u='', C='', s=''; let l=this.hoursDisabled||[]; E=new Date(this.viewDate); for (var y=0; y<24; y++) {
        E.setUTCHours(y); F=this.onRenderHour(E); if (l.indexOf(y)!==-1) {
            F.push('disabled');
        } let v=h(t, G, n, y); if ((v.valueOf()+3600000)<=this.startDate||v.valueOf()> this.endDate) {
            F.push('disabled');
        } else {
            if (A===y) {
                F.push('active');
    ...
    } this.picker.find('.datetimepicker-hours td').html(r.join('')); r=[]; u=''; C=''; s=''; let m=this.minutesDisabled||[]; E=new Date(this.viewDate); for (var y=0; y<60; y+=this.minuteStep) {
        if (m.indexOf(y)!==-1) {
            continue;
    ...
    } this.picker.find('.datetimepicker-minutes td').html(r.join('')); let J=this.date.getUTCFullYear(); let o=this.setTitle('.datetimepicker-months', t).end().find('.month').removeClass('active'); if (J===t) {
...
}r=''; t=parseInt(t/10, 10)*10; let H=this.setTitle('.datetimepicker-years', t+'-'+(t+9)).end().find('td'); t-=1; E=new Date(this.viewDate); for (var y=-1; y<11; y++) {
    E.setUTCFullYear(t); F=this.onRenderYear(E); if (y===-1||y===10) {
        F.push(b);
    }r+='<span class="'+F.join(' ')+'">'+t+'</span>'; t+=1;
}H.html(r); this.place();
}, updateNavArrows: function() {
    let m=new Date(this.viewDate), k=m.getUTCFullYear(), l=m.getUTCMonth(), j=m.getUTCDate(), i=m.getUTCHours(); switch (this.viewMode) {
    ...
    j.preventDefault(); j.stopPropagation(); if (this.wheelPause) {
        return;
    } this.wheelPause=true; let i=j.originalEvent; let l=i.wheelDelta; let k=l>0?1:(l===0)?0:-1; if (this.wheelViewModeNavigationInverseDirection) {
        k=-k;
    } this.showMode(k); setTimeout(d.proxy(function() {
        this.wheelPause=false;
    }, this), this.wheelViewModeNavigationDelay);
}, click: function(m) {
    m.stopPropagation(); m.preventDefault(); let n=d(m.target).closest('span, td, th, legend'); if (n.is('.'+this.icontype)) {
        ...
        } this.fill(); this.element.trigger({type: n[0].className+':'+this.convertViewModeText(this.viewMode), date: this.viewDate, startDate: this.startDate, endDate: this.endDate}); break; case 'clear':this.reset(); if (this.autoclose) {
            this.hide();
    ...
    if (!k||k==='date') {
        ...
        this.viewDate=i;
    } this.fill(); this.setValue(); let j; if (this.isInput) {
        j=this.element;
    } else {
        if (this.component) {
    ...
    if (!i) {
        return j;
    } let k=new Date(j.valueOf()); k.setUTCMinutes(k.getUTCMinutes()+(i*this.minuteStep)); return k;
}, moveHour: function(j, i) {
    if (!i) {
        return j;
    } let k=new Date(j.valueOf()); k.setUTCHours(k.getUTCHours()+i); return k;
}, moveDate: function(j, i) {
    if (!i) {
        return j;
    } let k=new Date(j.valueOf()); k.setUTCDate(k.getUTCDate()+i); return k;
}, moveMonth: function(j, k) {
    if (!k) {
        return j;
    } let n=new Date(j.valueOf()), r=n.getUTCDate(), o=n.getUTCMonth(), m=Math.abs(k), q, p; k=k>0?1:-1; if (m===1) {
        p=k===-1?function() {
    ...
    } let k=false, j, i, n; switch (o.keyCode) {
    ...
    let p=this.viewMode; this.showMode(-1); this.fill(); if (p===this.viewMode&&this.autoclose) {
        this.hide();
    }
} else {
    this.fill(); if (this.autoclose) {
    ...
    if (i) {
        let j=Math.max(0, Math.min(c.modes.length-1, this.viewMode+i)); if (j>=this.minView&&j<=this.maxView) {
    ...
    let i=Array(...arguments); i.shift(); let j; this.each(function() {
        let n=d(this), m=n.data('datetimepicker'), l=typeof k==='object'&&k; if (!m) {
    ...
    let i=l.replace(this.validParts(j), '\0').split('\0'), k=l.match(this.validParts(j)); if (!i||!i.length||!k||k.length===0) {
        throw new Error('Invalid date format.');
    } return {separators: i, parts: k};
}, parseDate: function(A, y, v, j, r) {
    if (A instanceof Date) {
        let u=new Date(A.valueOf()-A.getTimezoneOffset()*60000); u.setMilliseconds(0); return u;
    ...
    for (var x=0, w=y.parts.length; x<w; x++) {
        B=parseInt(q[x], 10); t=y.parts[x]; if (isNaN(B)) {
            ...
            case 'MM':k=d(e[v].months).filter(function() {
                let i=this.slice(0, q[x].length), s=q[x].slice(0, i.length); return i===s;
            }); B=d.inArray(k[0], e[v].months)+1; break; case 'M':k=d(e[v].monthsShort).filter(function() {
                let i=this.slice(0, q[x].length), s=q[x].slice(0, i.length); return i.toLowerCase()===s.toLowerCase();
    ...
    } var l=[], r=d.extend([], q.separators); for (let n=0, j=q.parts.length; n<j; n++) {
        if (r.length) {
            l.push(r.shift());
        }l.push(k[q.parts[n]]);
    } if (r.length) {
    ...
    let i=d(this); if (i.data('datetimepicker')) {
        return;
