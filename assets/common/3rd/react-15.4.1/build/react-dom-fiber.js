 ...
 */
            ...
            let REACT_ELEMENT_TYPE = _dereq_(4);

            let _require = _dereq_(2),
                REACT_COROUTINE_TYPE = _require.REACT_COROUTINE_TYPE,
                REACT_YIELD_TYPE = _require.REACT_YIELD_TYPE;

            let ReactFiber = _dereq_(5);
            let ReactReifiedYield = _dereq_(15);

            let cloneFiber = ReactFiber.cloneFiber,
            ...
            let createReifiedYield = ReactReifiedYield.createReifiedYield;
            ...
            // Will fix reconciliation properly later.
                            let clone = shouldClone ? cloneFiber(existingChild, priority) : existingChild;
                            if (!shouldClone) {
              // TODO: This might be lowering the priority of nested unfinished work.
                            ...
                            let child = createFiberFromElement(element, priority);
                    ...
                    case REACT_COROUTINE_TYPE:
                        {
                            let coroutine = newChildren;
                            let _child = createFiberFromCoroutine(coroutine, priority);
                    ...
                    case REACT_YIELD_TYPE:
                        {
                            let yieldNode = newChildren;
                            let reifiedYield = createReifiedYield(yieldNode);
                            let _child2 = createFiberFromYield(yieldNode, priority);
                    ...
                    if (isArray(newChildren)) {
                        ...
                        let existing = existingChild;
                        for (let i = 0; i < newChildren.length; i++) {
                            let nextExisting = existing && existing.sibling;
      ...
      // TODO: Throw for unknown children.
           ...
           */
                            let coroutine = newChildren;
                            let _child3 = createFiberFromCoroutine(coroutine, priority);
           ...
           */
                            let yieldNode = newChildren;
                            let reifiedYield = createReifiedYield(yieldNode);
                            let _child4 = createFiberFromYield(yieldNode, priority);
...
// nor polyfill, then a plain number is used for performance.
            let REACT_COROUTINE_TYPE = typeof Symbol === 'function' && Symbol['for'] && Symbol['for']('react.coroutine') || 0xeac8;

            let REACT_YIELD_TYPE = typeof Symbol === 'function' && Symbol['for'] && Symbol['for']('react.yield') || 0xeac9;
 ...
 */
            ...
            let ReactFiberReconciler = _dereq_(9);

            let warning = _dereq_(18);
  ...
  /* $FlowFixMe: Element should have this property. */
    ...
    /* As a result of the refinement issue this type isn't known. */
            ...
            let DOMRenderer = ReactFiberReconciler({
 ...
 */
            ...
            let ReactTypeOfWork = _dereq_(16);
...
//    compatible.
            let createFiber = function(tag, key) {
            return {
...
// TODO: Rename to createWorkInProgressFiber or something like that.
  ...
  // TODO: Pass in the new pendingProps as an argument maybe?
            ...
            let fiber = createFiber(HostContainer, null);
            return fiber;
        };
  ...
  // $FlowFixMe: ReactElement.key is currently defined as ?string but should be defined as null | string in Flow.
            let fiber = createFiberFromElementType(element.type, element.key);
    ...
    // Currently assumed to be a continuation and therefore is a fiber already.
            ...
            let fiber = createFiber(CoroutineComponent, coroutine.key);
            ...
            let fiber = createFiber(YieldComponent, yieldNode.key);
            fiber.pendingProps = {};
            return fiber;
        };
 ...
 */
        ...
        let ReactInstanceMap = _dereq_(13);
    ...
    // We now have clones. Let's store them as the currently progressed work.
      ...
      // continue from this point.
            ...
            let priorityLevel = workInProgress.pendingWorkPriority;
  ...
  // Class component state updater
            ...
            enqueueSetState: function(instance, partialState) {
                let fiber = ReactInstanceMap.get(instance);
                let updateQueue = fiber.updateQueue ? addToQueue(fiber.updateQueue, partialState) : createUpdateQueue(partialState);
                scheduleUpdate(fiber, updateQueue, LowPriority);
            },
            enqueueReplaceState: function(instance, state) {
                let fiber = ReactInstanceMap.get(instance);
                let updateQueue = createUpdateQueue(state);
                updateQueue.isReplace = true;
                scheduleUpdate(fiber, updateQueue, LowPriority);
            },
            enqueueForceUpdate: function(instance) {
                let fiber = ReactInstanceMap.get(instance);
                let updateQueue = fiber.updateQueue || createUpdateQueue(null);
    ...
    // Compute the state using the memoized state and the update queue.
            ...
            let previousState = current ? current.memoizedState : null;
            let state = updateQueue ? mergeUpdateQueue(updateQueue, previousState, props) : previousState;
      ...
      // Proceed under the assumption that this is a functional component
            ...
            let coroutine = workInProgress.pendingProps;
            if (!coroutine) {
 ...
 */
        ...
        let ReactTypeOfWork = _dereq_(16);
        let ClassComponent = ReactTypeOfWork.ClassComponent,
        HostContainer = ReactTypeOfWork.HostContainer,
        HostComponent = ReactTypeOfWork.HostComponent;

        let _require = _dereq_(12),
        callCallbacks = _require.callCallbacks;

        module.exports = function(config) {
        let updateContainer = config.updateContainer;
        let commitUpdate = config.commitUpdate;
        ...
        function commitWork(current, finishedWork) {
          ...
          // the update queue is empty.
                    ...
                    if (finishedWork.callbackList) {
                        let callbackList = finishedWork.callbackList;
          ...
          // TODO: Attach children to root container.
                    let children = finishedWork.output;
          ...
          // Commit the work prepared earlier.
                    ...
                    let _children = child && !child.sibling ? child.output : child;
 ...
 */
    ...
    module.exports = function(config) {
        ...
        let prepareUpdate = config.prepareUpdate;
  ...
  // of a sibling reads, then the next sibling updates and reads etc.
    ...
    // TODO: Compare this to a generator or opaque helpers like Children.
              ...
              let fn = coroutine.handler;
              let props = coroutine.props;
              let nextChildren = fn(props, yields);

              let currentFirstChild = current ? current.stateNode : null;
        ...
        // are invoked.
          ...
          // TODO: Is this actually ever going to change? Why set it every time?
              ...
              // This can happen when we abort work.
                    ...
                    let _instance = createInstance(workInProgress.type, newProps, children);
 ...
 */
    ...
    let _require = _dereq_(10),
          createFiberRoot = _require.createFiberRoot;

    let ReactFiberScheduler = _dereq_(11);
    ...
    module.exports = function(config) {
          ...
          return {
            mountContainer: function(element, containerInfo) {
                let root = createFiberRoot(containerInfo);
 ...
 */
    ...
    let ReactFiberBeginWork = _dereq_(6);
    let ReactFiberCompleteWork = _dereq_(8);
    let ReactFiberCommitWork = _dereq_(7);
  ...
  // and ReactFiberBeginWork. Don't know if there's a better way to do this.
        ...
        let _ReactFiberBeginWork = ReactFiberBeginWork(config, getScheduler),
            beginWork = _ReactFiberBeginWork.beginWork;

        let _ReactFiberCompleteWo = ReactFiberCompleteWork(config),
            completeWork = _ReactFiberCompleteWo.completeWork;

        let _ReactFiberCommitWork = ReactFiberCommitWork(config),
            commitWork = _ReactFiberCommitWork.commitWork;

        let scheduleAnimationCallback = config.scheduleAnimationCallback;
        let scheduleDeferredCallback = config.scheduleDeferredCallback;

  // The default priority to use for updates.
        let defaultPriority = LowPriority;
  ...
  // Linked list of roots with scheduled work on them.
    ...
    // TODO: Error handling.
            ...
            while (effectfulFiber) {
                let current = effectfulFiber.alternate;
                commitWork(current, effectfulFiber);
                let next = effectfulFiber.nextEffect;
    ...
    // not to do the work.
      ...
      // to the system not to redo any work here.
        ...
        // If we're at the root, there's no more work to do. We can flush it.
                    let _root = workInProgress.stateNode;
        ...
        // const hasMoreWork = workInProgress.pendingWorkPriority !== NoWork;
                    ...
                    let nextWork = findNextUnitOfWork();
    ...
    // Set the priority on the root, without deprioritizing
      ...
      // We're the only work scheduled.
        ...
        function performWithPriority(priorityLevel, fn) {
            let previousDefaultPriority = defaultPriority;
 ...
 */
    ...
    exports.mergeUpdateQueue = function(queue, prevState, props) {
        let node = queue;
        let state = queue.isReplace ? null : _assign({}, prevState);
        while (node) {
            let _partialState = void 0;
            if (typeof node.partialState === 'function') {
                let updateFn = node.partialState;
 ...
 */
    ...
    exports.createReifiedYield = function(yieldNode) {
        let fiber = createFiberFromElementType(yieldNode.continuation, yieldNode.key);
        return {
            continuation: fiber,
