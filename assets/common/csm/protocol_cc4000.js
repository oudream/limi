...
var CjChannelTcpclient = require('./../cjs/nodejs/cjchannel_tcpclient')
var fs = require('fs')
...
BasPacket.prototype.toBuffer = function () {
  ...
  var iTotalSize = self.getTotalSize()
  if (iTotalSize <= 0) {
    return Buffer.alloc(0)
  }
  var rBuf = Buffer.alloc(self.getTotalSize())
  var attr
  var value
  var idx = 0
  var iOffset = 0
...
BasPacket.prototype.preparePacket = function (buf) {
  ...
  var iOldSize = buf.length
  var r = Buffer.allocUnsafe(iOldSize * 2)
  var i = 1
  var j = 1
  r[0] = buf[0]
  while (i < iOldSize - 1) {
...
BasPacket.prototype.encodePacket = function (buf) {
  ...
  var r = Buffer.allocUnsafe(iOldSize + 1 + 4 + 4 + 4 + 1 + 1)
...
BasParser.prototype.doReceived = function (data) {
  ...
  if (data.length > (BasDefine.PACKAGE_MAX_BUF_SIZE - this.recvOffset)) {
    var iNoDeal = this.recvOffset - this.dealOffset
...
BasParser.prototype.dealCache = function () {
  var index = this.dealOffset
  var end = this.recvOffset
  var buf = this.recvCache
  while (index < end) {
    if (buf[index] === BasDefine.PACKAGE_REQ_START) {
      ...
      if (iEndPos === 0) {
        ...
        var iMsgLength = iEndPos - iStartPos + 1
        if (iMsgLength <= BasDefine.PACKAGE_MAX_REQ_LEN) {
          var iMsgEnd = this.repairMsg(iStartPos, iEndPos + 1)
...
BasParser.prototype.repairMsg = function (iStartPos, iEndPos) {
  var buf = this.recvCache
  var msgBuf = this.msgCache
  msgBuf[0] = buf[iStartPos]
  var i = iStartPos + 1, j = 1
  for (; i < iEndPos - 1; i++ , j++) {
    if (buf[i] === BasDefine.PACKAGE_REQ_TRANS) {
...
function BasProtocol () {
  ...
  var tcpclient = new CjChannelTcpclient()
  tcpclient.isAutoOpen = true
  tcpclient.isAutoHeartbeat = true

  var basParser = new BasParser()
...
BasProtocol.prototype.dealPacket = function (buf, iEnd) {
  ...
  var iOffset = 0
  var pkStart = buf[iOffset]// BasDefine.PACKAGE_REQ_START;
  iOffset += 1

  var pkCommandSeq = buf.readIntLE(iOffset, true)
  iOffset += BasDefine.PACKAGE_ITEM_REQ_LEN

  var pkCommand = buf.readIntLE(iOffset, true)
  iOffset += BasDefine.PACKAGE_ITEM_REQ_LEN

  var pkRequest = buf.readIntLE(iOffset, true)
  iOffset += BasDefine.PACKAGE_ITEM_REQ_LEN

  var packet = BasPacket.packets.get(pkCommand)
  if (packet) {
    var msgObj = packet.fromBuffer(buf, iOffset)
  ...
  var pkCrc = buf[iEnd - 2]// crc

  var pkEnd = buf[iEnd - 1]// = BasDefine.PACKAGE_REQ_END;
}
...
// Init User Packet
if (true) {
  var userLoginPacket = new BasPacket()
  ...
  var alarmReqPacket = new BasPacket()
  ...
  var alarmAnsPacket = new BasPacket()
...
BasProtocol.test1 = function () {
 var basProtocol = new BasProtocol();
 ...
 setTimeout(function () {
   // send packet
   var packet = BasPacket.userLoginPacket.toPacket("user1", "password1", "no1", 1001);
