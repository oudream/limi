...
define(['jquery', 'bootstrap', 'util', 'global', 'server', 'css!' + BASEURL + 'common/ccss/all.css', 'es5'], function ($, bootstrap, util, g, server) {
  util.log('app - 载入页面' + window.location.pathname)
	// 主题样式
  var themeCss = ['../../../common/ccss/main.css', '../../../common/3rd/bootstrap-2.3.2/css/bootstrap-2.3.2.css', '../../../common/ccss/page.css', 'smallIcon.css', 'icon.css']
  ...
  requirejs(themeCss.map(function (item) {
	...
	// 加载自适应布局
  var loadUixLatyou = function (callback) {
...
//				debugger;
  ...
  var common = {
	...
	// 获取页面控制器
  var pageController
  var code = $(document.body).attr('code')
  var bindOnload = function () {
    requirejs(['domready!'], function (doc) {
      util.log('app - 页面加载完成')

			// 若存在参数key,则将参数取出传入页面的onload方法中，并在页面关闭前清空参数；
      var argkey = util.GetQueryString('argkey')
      if (argkey) {
        var data = g.data(argkey)
        if (data) {
