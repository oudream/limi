...
define(['jquery', 'cjcommon', 'cjdatabaseaccess', 'cjajax', 'cache', 'utils'], function ($) {
  ...
  panelConfig.objInit = function (id, data, tableName) {
      ...
      if (data[i].visible !== 0) {
        if (data[i].propType === 5) {
          ...
          if (valueScopes === '') {
            ...
            let obj = JSON.parse(foreignKey)
            setSelectSql(obj.sql, select.id)
          } else {
            let aSelect = []
            let oSelect = {}
          ...
          if (data[i].defaultValue !== '' && data[i].defaultValue !== null) {
            ...
            if (obj.type === '6') {
              ...
              db.load(sql, function fn (err, vals) {
                if (err) {
                  ...
                  $('#my_radio' + vals[0].F_SCALE).attr('checked', true)
                }
              }, reqParam)
            }
          }
            ...
            // 设置正则表达式
          div.appendChild(input)
          if (data[i].regularExpression !== '' && data[i].regularExpression !== null) {
     ...
     */
  panelConfig.operationInit = function (id, data) {
      ...
      if (err) {
        ...
        if (arr[1] !== undefined) {
          ...
          for (let i = 0; i < vals.length; i++) {
            cjCommon.createElement('option', {'value': vals[i][$.trim(arr[0])], 'textContent': vals[i][$.trim(arr[1])]}, $('#' + id))
          }
        } else {
          cjCommon.createElement('option', {'value': '', 'textContent': '全部'}, $('#' + id))
          for (let i = 0; i < vals.length; i++) {
