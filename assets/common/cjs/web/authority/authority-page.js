...
define(['jquery', 'async', 'global', 'jqGrid', 'ztree', 'cjcommon', 'cjstorage', 'cjdatabaseaccess', 'cjajax', 'loadNode', 'structure', 'model', 'view', 'controller', 'utils', 'cache', 'jqGridExtension'], function($, async, g) {
    ...
    let loadTreeSql = 'select * from sinopec_authority';
    ...
    function mainTableInit(tbID, colNames, colModel) {
        tbID.jqGrid({
            ...
            onSelectRow: function(id) {
                ...
                let upSql = 'select sinopec_authority.*,sinopec_group_authority.Visible from sinopec_authority,sinopec_group_authority,`sinopec_group` where sinopec_authority.MenuID = sinopec_group_authority.AID and sinopec_group.ID = sinopec_group_authority.GID and sinopec_group_authority.GID = ' + ID;
                updateTree(upSql);
    ...
    function saveFunc(GID) {
        ...
        let nodes = treeObj.transformToArray(node);
        let AID = [];
        let insertSql = 'delete from sinopec_group_authority where GID = ' + GID + ';';
        ...
        for (let j = 0; j < AID.length; j++) {
            insertSql = insertSql + 'insert into sinopec_group_authority(GID,AID,Visible) ' +
          'values(' + GID + ',' + AID[j] + ',1);';
        }
