...
define(['jquery', 'async', 'global', 'jqGrid', 'ztree', 'cjcommon', 'cjstorage', 'cjdatabaseaccess', 'cjajax', 'loadNode', 'structure', 'model', 'view', 'controller', 'utils', 'cache', 'jqGridExtension'], function($, async, g) {
    ...
    let loadMainSql = 'SELECT sinopec_group.ID,sinopec_group.GroupName FROM `sinopec_group` order by GroupName;';
    let delGroupSql = 'DELETE FROM `sinopec_group` WHERE ID = ';
    let delUserGroupSql = 'DELETE FROM sinopec_user_group WHERE UID = ';
      ...
      // data: data,
          ...
          // mtype : "get",
            ...
            onSelectRow: function(id) {
                ...
                let loadChildSql = 'SELECT sinopec_user.ID,sinopec_user.UserName,sinopec_user.Name from sinopec_user,`sinopec_group`,sinopec_user_group where sinopec_user.ID = sinopec_user_group.UID and `sinopec_group`.id = sinopec_user_group.GID and GID = ' + ID + ' order by UserName';
    ...
    function delFunc(tbID, sql, sql1) {
        ...
        let deleteSql = 'delete from sinopec_group_authority where GID = ' + GID + ';' + sql + '\'' + del + '\'';
        if (sql1 !== undefined) {
