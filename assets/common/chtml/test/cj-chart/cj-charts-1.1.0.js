...
CjCharts.init = function (dom) {
    ...
    /** 同一个DIV，单一实例限制 */
    var _chart = echarts.getInstanceByDom(dom);
    if (_chart) {
        chart.chart = _chart;
    } else {
...
CjCharts.getCjOption = function (param) {
    ...
    if (param.dataZoom && param.dataZoom.type && param.dataZoom.start && param.dataZoom.end) {
        var dataZoom = CjChartsBase.newDataZoom(param.dataZoom.type,param.dataZoom.start,param.dataZoom.end);
...
function load (data){
    ...
    var optionStr = JSON.stringify(_option);
