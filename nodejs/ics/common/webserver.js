...
WebServer.run = function () {
    async function loadEthernetIP() {
        try {
            let values = await WebServer._mysql.queryPromise(`select EthernetIP from omc_neconfig where NeNo = '${WebServer.serverId}'`);
    ...
    async function loadRtBusNodeIpPort() {
        try {
            ...
            WebServer.cc4kConfig.RtBusNodeIp = '127.0.0.1';
            WebServer.cc4kConfig.RtBusNodePort = 0;
        }
    }
    ...
    async function loadOmcSignalurl() {
        try {
