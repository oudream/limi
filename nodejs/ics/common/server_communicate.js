...
function protocolListenerInit() {
    ...
    _rtbusProtocol.on(BasPacket.rtReqUpdlistPacket.commandCode, fnDealRt);
    _rtbusProtocol.on(BasPacket.rtReqUpdkeyPacket.commandCode, fnDealRt);
                        ...
                        data: function() {
                            ...
                            for (let i = 0; i < reqMeasures.length; i++) {
                                ...
                                if (resMeasure !== null && reqValue) {
                                    measureBuffer.writeIntLE(resMeasure.id, iOffset, 6, true); iOffset += 8;
                                    if (typeof(reqValue) === 'number') {
                                        if (Number.isInteger(reqValue)) {
                                            measureBuffer.writeIntLE(reqValue, iOffset, 6, true); iOffset += 8;
                                        } else {
                                            measureBuffer.writeDoubleLE(reqValue, iOffset, true); iOffset += 8;
                                        }
                                    } else {
...
function getOmcServerInfo() {
  ...
  // let defaultDb = server.dbManager.findDb(_host,_dsn);
    ...
    let sql4 = `select AppID from ha_appconfig where NodeID = '${sServerID}' and AppName = 'node.exe'`;
    let sql5 = '';
    let sql6 = `select ItemValue from omc_omcconfig where ItemNo = '266'`;
    let sql7 = `select EthernetIP from omc_neconfig where NeNo = '${sServerID}'`;
    ...
    async.auto({
        ...
        queryRtBusPort: function(callback) {
            defaultDb.load(sql6, function(err, vals) {
                callback(err, vals);
            });
        },
        queryAppId: function(callback) {
            defaultDb.load(sql4, function(err, vals) {
                callback(err, vals);
            });
        },
        queryIpPort: ['queryAppId', function(value, callback) {
        ...
        if (!value.queryIpPort) {
            ...
            localIp = value.queryIpPort[0].ip;
            localPort = Number(value.queryIpPort[0].port);
        }

        if (!value.queryRtBusIp) {
            RtBusIp = '0.0.0.0';
        } else {
            RtBusIp = value.queryRtBusIp[0].EthernetIP;
        }

        if (!value.queryRtBusPort) {
            RtBusPort = '0.0.0.0';
        } else {
            RtBusPort = Number(value.queryRtBusPort[0].ItemValue);
        }
        ...
        _rtbusProtocol.start({
            ...
            LocalPort: localPort,
            RemotePort: RtBusPort,
            RemoteIpAddress: RtBusIp,
        });
